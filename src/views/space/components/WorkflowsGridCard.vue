...
import type { WorkflowCard, WorkflowCardAction } from '@/models/personalSpace'
...
const handleAction = (actionKey: string | number | Record<string, any> | undefined) => {
  ...
  const key = actionKey as WorkflowCardAction
  if (key === 'delete') {
    Modal.confirm({
      title: '确认删除',
...
<template>
  <div class="space-card">
    <!-- 卡片头部 -->
    <div class="card-head">
      <img :src="item.icon" alt="workflow-icon" class="card-icon" />
      <div class="card-title-wrap">
        <div class="card-title-row">
          <span class="card-title">{{ item.name }}</span>
          <icon-check-circle-fill v-if="item.verified" class="verified-icon" />
        </div>
        <div class="card-model-info">{{ item.workflowName }} · {{ item.nodeCount }} 节点</div>
      </div>
